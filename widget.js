...
const TreasureHunt = (function() {
    ...
    const itemsData = [
        { title: '赤い薔薇', icon: '🌹', desc: 'トランシー家の庭園に咲く、クリムゾン・レッド色の薔薇。気高い気品を纏っている。' },
        { title: '謎の小瓶', icon: '🫙', desc: '不気味な液体が入った小瓶。開けると甘い香りと共に冷たい悪寒が走る。' },
        { title: '契約書', icon: '📜', desc: '領地の管理や会社の運営、数えきれないほどの利益関係をまとめている。' },
        ...
        { title: 'ブルーベルの花', icon: '💠', desc: 'あの日、あの時の…。アロイスの思い出が潜んだ鮮やかな花。' },
        { title: 'ファントム社のタグ', icon: '🏷️', desc: 'ファントム社の商品に付いてるタグ。多様なラインのタグに惹かれてコレクターも多いと言う。' },
        { title: '革製の旅行鞄', icon: '👝', desc: '女王の番犬としてイギリス中を駆け巡る際、常に携帯している重厚な鞄。' },
        { title: '色んな柄の猫', icon: '🐈', desc: '執事セバスチャンの周りを何故かうろうろする猫。なついてくる。追いかけてみるとセバスチャンの部屋へ向かってる。' },
        { title: '琥珀色のブランデー', icon: '🍸', desc: '大人の嗜みとして、夜な夜なグラスに注がれる芳醇で強いお酒。眠れない夜の旦那様に提供した執事が叱られたことも。' },
        { title: 'セイロンティー', icon: '☕', desc: 'ファントムハイヴ邸で常に最高の淹れ方で提供される、香り高い紅茶。シーロンじゃない。' },
        { title: 'ウィスキーグラス', icon: '🥃', desc: '氷がカランと音を立てる、冷たく澄んだクリスタルのグラス。屋敷に訪問した髭野郎がここに散らかして帰ったようだ。' },
        { title: '領収書', icon: '🧾', desc: 'ファントム社のおもちゃや菓子の大口取引の記録。莫大な金額が並ぶ。緊密に検討しないと予定になかった注文が入ってるかも。' },
        { title: '調査報告書', icon: '📋', desc: '裏社会の事件やファントム社の納品目録などに関する、極秘の調査データ。' },
        { title: '招待状の草案', icon: '📝', desc: 'ファントムハイヴ伯爵へ送る舞踏会への誘いを書いた手紙。何回かやり直したのか修正の跡が残っている。' },
        { title: 'オレンジジャム', icon: '🍊', desc: 'ルカと作ったとされる、甘酸っぱくも隠し味のありそうなジャム。使用人たちが煩くてキッチンに入れないため外で作った。' },
        { title: '英国貴族院の書類', icon: '📑', desc: '政治的謀略や裏の取引が記された、表に出せない極秘の公文書。こんなところに落ちていてもいい物か…？' },
        { title: '特製アーモンドクッキー', icon: '🍪', desc: '上品な甘さの中に、どこか毒っ気を感じさせる可愛らしいお菓子。かなりの量を用意して置いても瞬く間に消える。' },
        { title: '輝くクラゲの標本', icon: '🪼', desc: 'トランシー邸の怪しげな部屋に飾られている、幻想的な海の生物。脚が多いから執事は親しく思ったのかもしれない。' },
        { title: '窓辺の鉢植え', icon: '🪴', desc: '手入れが行き届きながらも、静寂に包まれた部屋でひっそりと育つ植物。小さいため芋を植えるとクロードの仲間のようになる。' },
        { title: '作家の手帳', icon: '🗒️', desc: 'ある有望な作家が持ち歩く手帳。物語のアイデアやある事件を解決するためのメモがあるようだ。' },
        { title: '万年筆', icon: '🖋', desc: 'クロードが日程を報告する時、書類の片隅に旦那様が落書きをしたり、常にシエルのポケットに眠っていたりのペン。いざとなると使える。' },
        { title: 'ロンドンの地図', icon: '🗺️', desc: 'ファントムハイヴ邸とトランシー邸を結んで線を描いた地図。シエルの落書きのせいで役割を果たせなくなった。' },
        { title: 'ローストビーフのサンド', icon: '🥪', desc: '執事が手際よく用意した、英国の伝統的で上品な軽食。ピクニックや学校のスポーツの休み時間に。' },
        { title: '古びた羊皮紙', icon: '📃', desc: '悪魔との契約や呪術に関する古い言い伝えが記された難解な古文書。城の奥の部屋に隠れていた。' },
        { title: '女王からの手紙', icon: '📄', desc: '「女王の番犬」へ下される、絶対不可避の極秘ミッションが記された書簡。' },
        { title: '満開のスイート・ブライアー', icon: '🌸', desc: 'どこか儚く、美しく咲いている花。いつものトランシー庭園では見かけないのに、シエルが飾ったのか？' },
        { title: 'アザラシのクッション', icon: '🦭', desc: 'アロイスの部屋に転がる抱き枕。名前は『シエル』。その名にした理由は色味が似てるからと言うが、真実は？' },
        { title: '朝刊新聞', icon: '📰', desc: 'ロンドン・ニュース。市内の怪奇事件や、広告、事情、噂を伝える紙面。ファントムハイヴ伯爵は登場頻度が高い。' },
        { title: '光り輝く宝石の王冠', icon: '👑', desc: '伯爵の気品あふれる象徴物の一つ。被ったことはないかも。シエルを迎える旦那様のコスチューム候補ではあった。' },
        { title: '古びた鍵', icon: '🗝️', desc: 'トランシー邸の開かずの扉、秘密の地下室を開ける鍵。扉はどこにあるだろうか。この鍵は使えるだろうか。' },
        { title: 'くまのぬいぐるみ', icon: '🧸', desc: 'ファントム社の商品を真似してクロードに作らせた海賊版。ビター・ベアになってるよ。' },
        { title: '象牙の櫛', icon: '🪮', desc: '美しい髪を艶やかに整えるために使われる、職人技の光る高級な櫛。伯爵たちは豚の毛の櫛を使うようで、これは誰のものなんだろう。' },
        { title: '高級アロマオイル', icon: '🧴', desc: '甘い香りで心を惑わせる、トランシー邸の退廃的な雰囲気を象徴する小瓶。落ち着けない夜に使うと効果があったりなかったり。' },
